...
int ShrinkQuasiBisimulation::initialize_groups(
    ...
    vector<int> &state_to_group, const vector<int> *seed) const {
    /*
      Initial partition: among states with the same seed class (or all states,
      if seed is null), group 0 holds all goal states; every distinct h value
      ...
      distinguishing them upfront. Separating by seed is required for
      soundness when seed is given (see header comment).
    */
    typedef unordered_map<long long, int> GroupMap;
    GroupMap key_to_group;
    int num_groups = 0;
    for (int state = 0; state < ts.get_size(); ++state) {
        ...
        bool is_goal = ts.is_goal_state(state);
        assert(!is_goal || h == 0);
        // Combine the seed class with (goal ? -1 : h) into a single key.
        // -1 is distinct from any non-negative h value.
        long long seed_class = seed ? (*seed)[state] : 0;
        long long h_part = is_goal ? -1 : h;
        long long key = seed_class * (static_cast<long long>(SENTINEL) + 2) + (h_part + 1);
        pair<GroupMap::iterator, bool> result =
            key_to_group.insert(make_pair(key, num_groups));
...
/*
  ...
  already hold the initial partition (see ShrinkQuasiBisimulation::
  initialize_groups); this function only ever splits groups further, so
  whatever distinctions the initial partition already encodes (h/goal, and
  optionally a seed class) are preserved in the result.
*/
StateEquivalenceRelation ShrinkQuasiBisimulation::refine_and_build_relation(
    const TransitionSystem &ts, const Distances &distances, int target_size,
    vector<int> &state_to_group, int num_groups) const {
    int num_states = ts.get_size();
...
StateEquivalenceRelation ShrinkQuasiBisimulation::compute_equivalence_relation(
    ...
    vector<int> state_to_group(ts.get_size());
    int num_groups = initialize_groups(ts, distances, state_to_group);
    return refine_and_build_relation(
        ts, distances, target_size, state_to_group, num_groups);
}

StateEquivalenceRelation
ShrinkQuasiBisimulation::compute_equivalence_relation_with_seed(
    const TransitionSystem &ts, const Distances &distances, int target_size,
    const vector<int> &seed, utils::LogProxy &) const {
    assert(distances.are_goal_distances_computed());
    assert(static_cast<int>(seed.size()) == ts.get_size());
    vector<int> state_to_group(ts.get_size());
    int num_groups = initialize_groups(ts, distances, state_to_group, &seed);
    return refine_and_build_relation(
        ts, distances, target_size, state_to_group, num_groups);
}
